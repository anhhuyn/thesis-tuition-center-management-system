...
import type { DailySession} from "../../../utils/types/session";
...
export const WeeklyScheduleView: React.FC<WeeklyScheduleViewProps> = ({
  ...
  const [viewType, setViewType] = useState<"schedule" | "room">("schedule");
  ...
  const generateTimeSlots = (sessions: DailySession[]): TimeSlot[] => {
    ...
    try {
      const weekData: WeekDay[] = await Promise.all(
        weekDates.map(async (date) => {
          ...
          const sessions = await sessionApi.getSessionsByDate(dateStr);
  ...
  const getSessionsAtTime = (sessions: DailySession[], time: string): DailySession[] => {
  ...
  const getSessionsByRoomAndTime = (sessions: DailySession[], roomName: string, time: string): DailySession[] => {
    return sessions.filter(session => {
      const startTime = session.startTime.substring(0, 5);
      return session.roomName === roomName && startTime === time;
    });
  };
  ...
  // Modern color palette - more subtle and professional
  const getSessionBgColor = (session: DailySession): string => {
    ...
    if (session.status === 'canceled') return 'bg-gray-50 border-gray-200 line-through text-gray-400';
    return 'bg-blue-50 border-blue-200 hover:bg-blue-100/60';
  };

  // Compact session card with smaller size
  const renderSessionCard = (session: DailySession, day: WeekDay, showConflictIcon: boolean = false) => {
    ...
    return (
      ...
      >
        ...
        <div className="space-y-1">
          ...
          {session.teacherLeaveInfo && session.teacherLeaveInfo.type !== 'NONE' && !isCanceled && (
            <div className="flex items-center gap-1 mt-1.5 px-1.5 py-0.5 bg-rose-50 rounded-md">
  ...
  // Modern schedule view with compact sizing
                    ...
                    >
                      {sessions.length > 0 ? (
                        <div className="space-y-1.5">
                          {sessions.map(session => renderSessionCard(session, day, hasMultiple))}
                        </div>
                      ) : (
  ...
  // Modern room view with compact design
  const renderRoomView = () => {
    const [selectedDay, setSelectedDay] = useState<WeekDay | null>(weekDays[0] || null);
    ...
    const getRoomSessions = (roomName: string, time: string): DailySession[] => {
      return selectedDay.sessions.filter(session => 
        session.roomName === roomName && 
        session.startTime.substring(0, 5) === time
      );
    };
    
    return (
      <div className="space-y-5">
        {/* Day selector - modern pill design */}
        <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-thin">
          {weekDays.map((day, idx) => (
            <button
              key={idx}
              onClick={() => setSelectedDay(day)}
              className={`relative px-4 py-2 rounded-lg whitespace-nowrap transition-all duration-200 ${
            ...
            >
              ...
              <div className={`text-[10px] mt-0.5 font-medium ${
                ...
                {day.dayNumber}/{day.date.getMonth() + 1}
              </div>
            </button>
          ))}
        </div>
        
        {/* Room filter - modern select */}
        <div className="relative max-w-xs">
          <Filter className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-gray-400 stroke-[1.5]" />
          <select
            value={selectedRoom}
            onChange={(e) => setSelectedRoom(e.target.value)}
            className="w-full pl-9 pr-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-400 bg-white shadow-sm transition-all duration-200 appearance-none cursor-pointer hover:border-gray-300"
          >
        ...
        {/* Room schedule table - compact */}
        <div className="overflow-x-auto rounded-xl border border-gray-200 bg-white shadow-sm">
          <div className="min-w-[600px]">
            <div className="grid border-b border-gray-200 bg-gray-50/50" style={{ gridTemplateColumns: `80px repeat(${filteredRooms.length}, minmax(220px, 1fr))` }}>
              <div className="p-3 border-r border-gray-200 bg-gray-50/30 sticky left-0 font-semibold text-[10px] text-gray-500 uppercase tracking-wider">
                Giờ
              </div>
              {filteredRooms.map(room => (
                <div key={room.id} className="p-3 border-r border-gray-200 last:border-r-0">
                  <div className="font-semibold text-sm text-gray-800">{room.name}</div>
                  <div className="text-[10px] text-gray-500 mt-0.5">{room.seatCapacity} chỗ</div>
                </div>
              ))}
            </div>
            ...
            <div>
              {timeSlots.map((slot, slotIdx) => (
                <div key={slotIdx} className="grid border-b border-gray-100 last:border-b-0 transition-colors hover:bg-gray-50/30" style={{ gridTemplateColumns: `80px repeat(${filteredRooms.length}, minmax(220px, 1fr))` }}>
                  <div className="p-3 border-r border-gray-200 bg-gray-50/20 sticky left-0">
                    <div className="text-xs font-medium text-gray-600 tabular-nums">{slot.time}</div>
                  </div>
                  
                  {filteredRooms.map(room => {
                    const sessions = getRoomSessions(room.name, slot.time);
                    const hasMultiple = sessions.length > 1;
                    return (
                      <div key={room.id} className="p-1.5 border-r border-gray-100 last:border-r-0 min-h-[90px]">
                        {sessions.length > 0 ? (
                          <div className="space-y-1.5">
                            {sessions.map(session => renderSessionCard(session, selectedDay, hasMultiple))}
                          </div>
                        ) : (
                          <div className="flex items-center justify-center h-full min-h-[78px]">
                            <span className="text-gray-300 text-xs select-none">—</span>
                          </div>
                        )}
                      </div>
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden transition-all duration-200">
      {/* Header with sticky and clean design */}
      ...
      <div className="p-5 bg-white">
        {viewType === "schedule" ? renderScheduleView() : renderRoomView()}
        ...
        {weekDays.length > 0 && timeSlots.length === 0 && (
          <div className="text-center py-16">
            ...
            <div className="text-gray-500 font-medium">Không có lịch học trong tuần này</div>
            <div className="text-xs text-gray-400 mt-1">Hãy thử chọn tuần khác</div>
          </div>
        )}
      </div>
      
      {/* Modern legend with refined colors */}
      <div className="p-4 border-t border-gray-200 bg-gray-50/30 rounded-b-2xl">
        <div className="flex flex-wrap items-center gap-4 text-[11px]">
          ...
          <div className="flex items-center gap-2">
            <AlertCircle className="w-3 h-3 text-rose-500 stroke-[1.5]" />
            <span className="text-gray-600 font-medium">Cần xử lý</span>
          </div>
          <div className="flex items-center gap-2">
            <Layers className="w-3 h-3 text-amber-500 stroke-[1.5]" />
            <span className="text-gray-600 font-medium">Trùng giờ</span>
          </div>
        </div>
      </div>
    </div>
